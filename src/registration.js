...
function createRegisterPanel(member) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      `⚠️ 若你是伺服器擁有者、管理員，或你的最高身分組高於機器人，Discord 將無法讓 Bot 自動改名，請依照 Bot 提供的格式自行修改。\n\n` +
...
function createExistingMemberRegisterPanel(member) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      `⚠️ 管理員、伺服器擁有者，或身分組高於 Bot 的成員，請依照完成後顯示的格式自行修改暱稱。`
    )
    .setThumbnail(
      member.user.displayAvatarURL({
        extension: "png",
...
function createJobSelect(
  customId = "character_register_job",
  placeholder = "請選擇主要職業"
) {
...
function createRegisterModal(job) {
  ...
  const extraInput = new TextInputBuilder()
    ...
    .setLabel("其他角色（選填）")
    .setPlaceholder("例如：135主教/120夜使者")
    .setStyle(TextInputStyle.Short)
    .setRequired(false)
    .setMaxLength(80);
...
function createLevelUpdateModal() {
  const modal = new ModalBuilder()
    .setCustomId("character_update_level_modal")
    .setTitle("更新角色等級");

  const levelInput = new TextInputBuilder()
    .setCustomId("character_new_level")
    .setLabel("新的角色等級")
    .setPlaceholder("例如：168")
    .setStyle(TextInputStyle.Short)
    .setRequired(true)
...
async function ensureCharacterSheet(sheets, spreadsheetId) {
  ...
  const exists = metadata.data.sheets?.some(
    sheet =>
      sheet.properties?.title === CHARACTER_SHEET_NAME
  );
...
async function getCharacterRegistration(
  sheets,
  spreadsheetId,
  guildId,
  userId
...
async function saveCharacterRegistration(
  sheets,
  spreadsheetId,
  data
) {
  ...
  const existing =
    await getCharacterRegistration(
      sheets,
      spreadsheetId,
      data.guildId,
      data.userId
  ...
  if (existing) {
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range:
        `${CHARACTER_SHEET_NAME}!A` +
        `${existing.rowNumber}:I` +
        `${existing.rowNumber}`,
      valueInputOption: "RAW",
      requestBody: {
        values
...
async function sendRegistrationLog(
  interaction,
  data,
  action = "角色登記完成"
) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "主要角色",
        value: `${data.level}${data.job}`,
        inline: true
      },
      {
        name: "其他角色",
        value:
          data.extraCharacters || "無",
        inline: false
      },
      {
        name: "完整暱稱",
...
function createRegisteredMemberPanel(
  user,
  data
) {
  const embed =
    formatCharacterEmbed(user, data)
      .setDescription(
        "你已經完成角色登記，" +
        "可以使用下方按鈕更新資料。"
      );
      ...
      try {
        ...
        if (
          ...
          if (data) {
            ...
            await message.channel.send(
              createExistingMemberRegisterPanel(
                message.member
              )
            );
          }
        ...
        if (
          ...
          await saveCharacterRegistration(
            sheets,
            spreadsheetId,
            data
          );
